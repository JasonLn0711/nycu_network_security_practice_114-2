...
def render_markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Scan Engine",
        "",
        f"- Hash pre-check: `{_md(_scan_metadata(report, 'hash_filter', 'unknown'))}`",
        f"- Hash filter items: `{_md(_scan_metadata(report, 'hash_filter_items', 0))}`",
        f"- Hash filter bits: `{_md(_scan_metadata(report, 'hash_filter_bits', 0))}`",
        f"- Hash filter policy: `{_md(_scan_metadata(report, 'hash_filter_policy', 'unknown'))}`",
