# Course Overview

Source file: [csic30094-syllabus.docx](../handouts/raw/csic30094-syllabus.docx)

This note keeps the course-level facts from the official syllabus easy to find in plain text. Treat the `.docx` file as the raw source of truth.

## Course Facts

| Field | Value |
| --- | --- |
| University | National Yang Ming Chiao Tung University |
| Course code | `CSIC30094` |
| Course | Network Security Practice |
| Offering department | 網工所 |
| Class time / room | `W34-EC016[GF]` |
| Lecturer | 吳育松 |
| Credits | `3.00` |
| Type | 選修 |
| Prerequisite | Undergraduate-level operating systems and networking courses |

## Course Focus

The syllabus frames the course around security attacks and defense mechanisms, with real-world examples, team projects, mini-report presentations, and demos.

Topic areas listed in the syllabus:

- Introduction
- Vulnerabilities and exploits
- Crypto primitives
- Network security
- Host security
- Code integrity
- Web security

## Evaluation

| Component | Weight |
| --- | --- |
| Homework | `70%` |
| Term report and demo | `30%` |

## Reference Books

- Matt Bishop, `Computer Security`, 2nd edition, Addison-Wesley Professional, 2018.
- James Forshaw, `Attacking Network Protocols: A Hacker's Guide to Capture, Analysis, and Exploitation`, No Starch Press, 2017.

## Weekly Dates

The official syllabus lists the following Wednesday class dates:

| Week | Date |
| --- | --- |
| 1 | 2026-02-25 |
| 2 | 2026-03-04 |
| 3 | 2026-03-11 |
| 4 | 2026-03-18 |
| 5 | 2026-03-25 |
| 6 | 2026-04-01 |
| 7 | 2026-04-08 |
| 8 | 2026-04-15 |
| 9 | 2026-04-22 |
| 10 | 2026-04-29 |
| 11 | 2026-05-06 |
| 12 | 2026-05-13 |
| 13 | 2026-05-20 |
| 14 | 2026-05-27 |
| 15 | 2026-06-03 |
| 16 | 2026-06-10 |

## Routing Notes

- Keep the raw syllabus in `handouts/raw/`.
- Keep lecture-specific notes in `lectures/weekXX-*/`.
- Keep exam recall material in the matching lecture folder.
- Keep assignment-specific material under `homeworks/`.
- Keep term-project briefs, lab bundles, report assets, and demo evidence under `projects/`.
