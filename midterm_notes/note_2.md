...
Set-UID approach 比較像：

1. 使用者帶著自己的 shell 環境啟動高權限程式
2. 高權限程式一開始就可能吃到使用者帶來的東西
3. 這些東西可能是 environment、working directory、fd、`PATH`
4. 只要哪裡沒處理好，就可能被利用

service approach 比較像：

1. 高權限服務自己在乾淨環境裡啟動
2. 使用者不能把整包自己的執行環境帶進去
3. 使用者只能送固定格式的 request
4. 服務只處理被允許的那一小部分動作

### Android 為什麼常被拿來當例子？

因為 Android 的想法通常比較接近後者：

- App 先被 sandbox 起來
- App 不直接去跑一個 root 的 Set-UID 程式
- App 想做敏感操作時，要去找 system service
- system service 先檢查權限，再決定要不要幫它做

白話講就是：

**不要把總鑰匙直接交給 App。**
**要讓 App 只能透過櫃台申請，由系統服務代辦。**
