...
```mermaid
sequenceDiagram
    participant A as Subject A
    participant T as Trojan Horse
    participant F as File F
    participant G as File G
    participant B as Subject B

    Note over F: A 可讀可寫
    Note over G: A 可寫, B 可讀
    Note over B,F: B 本來不能直接讀 F

    A->>T: 執行 Trojan Horse
    T->>F: 以 A 的合法 read 權限讀取 F
    F-->>T: 回傳 F 的內容
    T->>G: 以 A 的合法 write 權限把內容寫入 G
    B->>G: 讀取 G
    G-->>B: 回傳從 F 複製來的內容
```
