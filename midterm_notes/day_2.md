...
Access Control Models 就是在回答：
**系統要怎麼決定「誰」可以對「哪個資源」做「哪些操作」**。

### 考試版定義

Access control models provide a framework for specifying and enforcing which subjects may access which objects under what rights or conditions.
...
> 如果我對某個檔案只有 read，沒有 write，也不能 execute，那我應該沒辦法把它弄出去吧？

其實**不一定**。

因為「複製檔案」這件事，通常不是去修改原本那個檔案，而是：

1. **先讀出來源檔案的內容**
2. **再把內容寫到另一個地方**

所以對一般檔案來說，若一個 subject：
...
也就是說，**複製來源檔案本身，通常不需要對來源檔案有 write 權限；也不需要 execute 權限。**

真正危險的點是：

* 你能不能**讀到機密資料**
* 你又能不能把它**寫到另一個 object**

只要答案是「可以」，Trojan Horse 就可能利用這條合法權限路徑做資訊外洩。

如果再講更精確一點：

* 來源路徑通常還需要對相關目錄有 **search / execute** 權限，才能走到那個檔案
* 目的位置通常需要對目錄有 **write** 與 **search / execute** 權限，才能建立新檔或覆寫檔案

所以 DAC 的弱點不是只有「原檔會不會被改壞」，
而是**只要合法權限允許「read here, write there」，資訊就可能被搬運出去**。
...
BLP 的大問題不是它完全沒用，而是：
**它很會管「某一個瞬間的合法讀寫」，但不夠會管「整個過程中資訊到底有沒有偷偷流出去」。**

### 申論題可以怎麼想

你可以用這個順序答：

1. 先講 **BLP 是 state-based**
2. 所以它對 **state sequence** 的掌握不夠
3. 因此會出現 **not sufficient** 與 **not necessary**
4. 再補充它 **只保護 confidentiality，不保護 integrity**
5. 最後收尾：**也擋不住 covert channels**

### 最短定義版

BLP 的缺陷是：

* 它用「單一 secure state」定義安全，但這不足以完整描述真實系統中的資訊流。
* 因此它 **不充分（not sufficient）**，也 **非必要（not necessary）**。
* 它只處理 **confidentiality**，不處理 **integrity**。
* 它無法消除 **covert channels**。
...
Not sufficient 的意思是：
**就算系統每一個當下都符合 BLP 規則，也不保證整個執行過程真的沒有資訊外洩。**
...
一個分析員先在高等級工作階段讀到機密報告，之後切到較低等級的工作階段，靠記憶或暫存在程式中的內容，把重點改寫到低等級文件或 email 草稿裡。

每一步單看都可能像是合法操作，但整體上就是把 high 的資訊帶到 low。

### Workflow

1. subject 在 high level 讀取機密檔案
2. subject 降低 current level，或切到 low session
3. subject 把剛才看到的內容寫進 low object
4. low-level 使用者讀到這份內容
5. 結果：整段 sequence 發生了 illegal information flow
...
Not necessary 的意思是：
**一個系統如果不完全符合 BLP 的形式條件，也不代表它一定不安全。**
...
真實系統裡常有 trusted subject，例如經過授權的人工審核員、sanitizer、declassifier。
它會先讀高等級文件，刪掉敏感內容後，再發布低等級版本。

這種「降級後釋出」在嚴格 BLP 看起來像 write down，但實務上如果流程可驗證、內容已脫敏，就未必不安全。

### Workflow

1. trusted subject 讀取 high document
2. 將內容去識別化、刪除機密欄位、產生摘要
3. 經過審核後輸出到 low document
4. 低等級人員只能看到已清洗版本
5. 結果：形式上碰到 write down，但實際上未必造成洩密
...
一個系統可以完全沒有洩密，但內容早就被亂改、灌假資料、污染掉了。
這不是 BLP 的處理範圍。

### Real-world example

低可信來源的程式或使用者，把錯誤資料、惡意指令、污染過的紀錄寫進高等級資料庫或高權限系統。

從 confidentiality 角度看，不一定有問題；但從 integrity 角度看，系統已經被汙染。

### Workflow

1. low-integrity subject 產生錯誤或惡意資料
2. 這些資料被送進 high-level / high-value system
3. 高等級系統接受並使用了這些資料
4. 結果：資料正確性被破壞，但 BLP 本身沒有解決這件事

### 補一句考試很好用

如果題目問你「那 integrity 要靠誰？」你可以補：
**BLP 對 confidentiality；Biba 對 integrity。**
...
Covert channel 是：
**不走正常授權的讀寫路徑，而是利用共享資源或系統狀態偷偷傳遞資訊。**
...
* BLP 在管「正門」
* covert channel 走的是「旁門、暗號、側面訊號」

所以就算正規 read/write 都被管住，資訊還是可能從別的地方漏出去。

### Real-world example

高等級 process 用 CPU load、lock 狀態、磁碟空間、記憶體占用、印表機佇列長度來編碼 0/1；
低等級 process 不直接讀機密檔案，只觀察這些變化，就能把 secret bit 一點一點解回來。

### Workflow

1. high subject 先把 secret 編成 bit sequence
2. bit = 1 時大量占用某資源；bit = 0 時不占用
3. low subject 持續觀察該資源狀態
4. low subject 根據觀察結果還原 bit
5. 結果：沒有明確違反正常 read/write 路徑，卻還是洩密

### 一句話收尾

**BLP 能限制 overt channel，但不能根除 covert channel。**

### 申論題作答模板

如果老師要你寫長一點，可以直接照下面答：

> Bell-LaPadula has several important limitations. First, it is state-based, so security of individual states is not sufficient to guarantee security of an entire execution sequence. For the same reason, its conditions are also not necessary, because some executions that violate the formal rules may still be safe in practice, especially when trusted subjects perform sanitization or declassification. Second, BLP focuses only on confidentiality and does not address integrity. Third, it cannot eliminate covert channels, where information is transmitted through shared resources or side effects rather than normal authorized read/write operations.
