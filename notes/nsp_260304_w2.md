...
這是一份為您整理的「網路安全實務-攻擊與防禦」（吳育松教授）第二週下半部課程講義。

本堂課從上半部的「環境變數攻擊面」延伸，正式進入作業系統安全防禦的核心核心：**存取控制 (Access Control)** 與 **權限管理機制**。教授詳細解說了 Linux/Unix 系統如何辨識使用者身分、如何解決一般使用者需要執行特權操作的兩難（Set-UID 機制），以及最基礎卻也最重要的檔案權限管理系統。

以下為您梳理專有名詞、底層運作邏輯、現實應用實例，並結合您的高階資安背景與系統開發進行延伸補充。

---

## 一、 系統安全基石：存取控制與使用者身分 (Access Control & User ID)
